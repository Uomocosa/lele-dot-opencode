...
| **Multi-language tool** | `*-(language_fullname)` e.g. `grpc-rust`, `libp2p-rust`, `libp2p-python` | `~/.config/opencode/skills/` | Filtered per-project via permissions |
...
The `*-(language_fullname)` pattern is for tools available in multiple languages (libp2p, gRPC, etc.). Unlike the compact `*-rs`/`*-py` suffixes, use the full language name so the tool name is distinct.

**Tool permission rule:** Tool skills — both bare-name (`pixi`) and multi-language (`libp2p-rust`) — are NOT matchable by glob patterns. They must be listed by their exact full name in `permission.skill`:
```json
{ "pixi": "allow", "libp2p-rust": "allow" }
```
